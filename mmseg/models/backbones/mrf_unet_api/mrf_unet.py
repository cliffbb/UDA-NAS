# Obtained from https://github.com/zifuwanggg/MRF-UNets
import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F

from .ops import Down, Up, Cat, Head, Filter, ChoiceBlock
...
class SuperNet(nn.Module):
    def __init__(self, image_channels, num_classes, channel_step=5):
        ...
        self.channels = [self.image_channels] + [int(c[-1]) for c in self.configurations]
        ...
        for i in range(self.layers - 1):
            ...
            if i in self.down_idx:
                block.append(Down(self.channels[i], self.channels[i+1], supernet=True))
            elif i in self.up_idx:
                block.append(Up(self.channels[i], self.channels[i+1], supernet=True))
            elif (i - 1) in self.up_idx:
                idx = self.step - self.up_idx.index(i-1) - 1
                in_channels = self.channels[i] + self.channels[self.down_idx[idx]]
                for kernel_size in self.kernel_list:
                    block.append(ChoiceBlock(in_channels, self.channels[i+1], kernel_size=kernel_size, stride=1, supernet=True))
            else:
                for kernel_size in self.kernel_list:
                    block.append(ChoiceBlock(self.channels[i], self.channels[i+1], kernel_size=kernel_size, stride=1, supernet=True))

            self.block.append(block)

        self.cat = Cat()
        self.filter = Filter()
        self.head = Head(self.channels[-1], self.num_classes)
...
class ChildNet(nn.Module):
    def __init__(self, image_channels, num_classes, channel_step, choices):
        ...
        self.channels = [self.image_channels] + \
        ...
        for i in range(self.layers - 1):
            if i in self.down_idx:
                self.block.append(Down(self.channels[i], self.channels[i+1]))
            elif i in self.up_idx:
                self.block.append(Up(self.channels[i], self.channels[i+1]))
            elif (i - 1) in self.up_idx:
                kernel_size = self.kernel_list[kernel_choices[i]]
                idx = self.step - self.up_idx.index(i-1) - 1
                in_channels = self.channels[i] + self.channels[self.down_idx[idx]]
                self.block.append(ChoiceBlock(in_channels, self.channels[i+1], kernel_size=kernel_size))
            else:
                kernel_size = self.kernel_list[kernel_choices[i]]
                self.block.append(ChoiceBlock(self.channels[i], self.channels[i+1], kernel_size=kernel_size))
