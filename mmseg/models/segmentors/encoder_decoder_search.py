...
class EncoderDecoderSearch(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(EncoderDecoderSearch, self).__init__(init_cfg)
    ...
    def extract_feat(self, img, choices_one_hot):
        """Extract features from images."""
        x = self.backbone(img, choices_one_hot)
        if self.with_neck:
            x = self.neck(x)
        return x

    def generate_pseudo_label(self, img, img_metas, choices_one_hot):
        self.update_debug_state()
        if self.debug:
            self.debug_output = {'Image': img,}
        out = self.encode_decode(img, img_metas, choices_one_hot)
        if self.debug:
    ...
    def encode_decode(self, img, img_metas, choices_one_hot, upscale_pred=True):
        """Encode images with backbone and decode into a semantic segmentation
        map of the same size as input."""
        x = self.extract_feat(img, choices_one_hot)
    ...
    def forward_with_aux(self, img, img_metas, choices_one_hot):
        self.update_debug_state()
    ...
    def forward_dummy(self, img, choices_one_hot):
        """Dummy forward function."""
        seg_logit = self.encode_decode(img, None, choices_one_hot)
    ...
    # TODO refactor
    def slide_inference(self, img, img_meta, choices_one_hot, rescale):
        ...
        if batched_slide:
            ...
            crop_seg_logits = self.encode_decode(crop_imgs, img_meta, choices_one_hot)
            ...
            for h_idx in range(h_grids):
                for w_idx in range(w_grids):
                    ...
                    x1 = max(x2 - w_crop, 0)
                    crop_img = img[:, :, y1:y2, x1:x2]
                    crop_seg_logit = self.encode_decode(crop_img, img_meta, choices_one_hot)
                    preds += F.pad(crop_seg_logit,
                                   (int(x1), int(preds.shape[3] - x2), int(y1),
                                    int(preds.shape[2] - y2)))
    ...
    def whole_inference(self, img, img_meta, choices_one_hot, rescale):
        """Inference with full image."""

        seg_logit = self.encode_decode(img, img_meta, choices_one_hot)
        if rescale:
    ...
    def inference(self, img, img_meta, rescale):
        ...
        """    
        choices_one_hot = self.choices_one_hot
        
        assert self.test_cfg.mode in ['slide', 'whole']
        ori_shape = img_meta[0]['ori_shape']
        assert all(_['ori_shape'] == ori_shape for _ in img_meta)
        if self.test_cfg.mode == 'slide':
            seg_logit = self.slide_inference(img, img_meta, choices_one_hot, rescale)
        else:
            seg_logit = self.whole_inference(img, img_meta, choices_one_hot, rescale)
