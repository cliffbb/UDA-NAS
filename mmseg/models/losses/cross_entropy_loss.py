...
class RecallCrossEntropyLoss(nn.Module):
    ...
    def forward(self, cls_score, label, reduction_override=None, **kwargs):
        """Forward function."""
        assert reduction_override in (None, 'none', 'mean', 'sum')
        ...
        idex = (pred != label).view(-1) 
        
        #calculate ground truth counts
        gt_counter = torch.ones((self.n_classes,)).cuda() 
        gt_idx, gt_count = torch.unique(label, return_counts=True)
        ...
        CE = F.cross_entropy(cls_score, label, reduction=reduction, ignore_index=self.ignore_index)
        
        loss =  (weight[label] * CE).mean()
