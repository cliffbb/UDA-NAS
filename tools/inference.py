...
from mmseg.models.backbones.mrf_unet_api.mrf_unet import MRFSuperNet, ChildNet
from mmseg.models.backbones.mrf_unet_api.mrf import MRF
...
def get_args():
    ...
    parser.add_argument('--gamma-iter', type=int, default=10)
    parser.add_argument('--flops-path', type=str, default="flops.pkl")
    parser.add_argument('--ckp-path', type=str, default="/path/to/search/checkpoint.pth")
...
def main(args):
    ...
    pretrained_dict = {k.replace('model.backbone.mrfunet.', ''): v 
                for k, v in pretrained_dict.items()
                if k.replace('model.backbone.mrfunet.', '') in model_dict}
...
def inference(mrf): 
    ...
    for (key, value) in mrf.unary.items():
        # solution.append(np.where(int(value) == mplp_query[key])[0][0])
        solution.append((mplp_query[key]))
...
def solve_gamma(supernet, potentials, solutions, i):
    ...
    while flops > args.target:
        ...
        gamma_max *= 2
        mrf = MRF(supernet=supernet, potentials=potentials, gamma=gamma_max, 
                  lams=args.lams[:i], solutions=solutions[:i], flops_path=args.flops_path)
    ...
    for iter in range(args.gamma_iter):
        ...
        mrf = MRF(supernet=supernet, potentials=potentials, gamma=gamma_mid, 
                  lams=args.lams[:i], solutions=solutions[:i], flops_path=args.flops_path)
        flops = inference(mrf)[1]
